"""
Authenticator for Omega P06 v1.2
...
class OMGP06Authenticator(AuthenticateBase):
    """
    Authentication model for Omega P06 v1.2
    """
    ...
    def __init__(self, device: str):
        ...
        Args:
            model: OMGP06Architecture instance
    ...
    def forward(self, image: Image.Image) -> AuthResult:
        """
        Authenticate Omega P06 v1.2 part
    ...
    def get_prob(self, probs: torch.Tensor) -> Tuple[float, str]:
        ...
        prob_value = probs[index].item()

        if index == 0:  # fake
            confidence = min(0.5, 1.0 - prob_value)
        elif index == 1:  
            confidence = max(0.6, prob_value)
        else:
            confidence = prob_value
        return confidence, f"{confidence:.4f}-{label}"
