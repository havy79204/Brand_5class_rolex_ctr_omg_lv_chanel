...
class OMGP01Detector(DetectorBase):
    ...
    def crop(self, image, position, class_id=None) -> Image.Image:
        ...
        Logic:
        1. Calculate square size from max(box_w, box_h)
        2. ALWAYS add padding to make box square (don't crop on original image)
        3. Crop square box from padded image
        ...
        Returns:
            PIL Image - square cropped image (always has padding if box is not square)
        """
        x1, y1, x2, y2 = position
        box_w = x2 - x1
        box_h = y2 - y1
        
        # STEP 1: Crop ORIGINAL box first (rectangle)
        cropped_original = self.img_preparing_yolo[y1:y2, x1:x2]
        
        # STEP 2: Calculate padding needed to make square
        ...
        if box_w == box_h:
            ...
            if box_w > box_h:
                ...
                pad_bottom = size - box_h - pad_vertical
                pad_left = 0
                pad_right = 0
            else:
                pad_horizontal = (size - box_w) // 2
                pad_left = pad_horizontal
                pad_right = size - box_w - pad_horizontal
                pad_top = 0
                pad_bottom = 0
